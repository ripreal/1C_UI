...
Процедура ЗагрузитьКурсыТоварныхРынков(КлиенсткийСертификат = Неопределено) Экспорт
	...
	#Иначе
		...
		ПараметрыЗапроса = ИнтерфейсHTTPЗапросовКлиентСервер.ПараметрыHTTPЗапроса();
		...
		Для каждого Товар Из ЗагружаемыеТовары Цикл
			...
			ТекстОтвета = ИнтерфейсHTTPЗапросовКлиентСервер.ПолучитьОтветHTTPЗапросаЗащищенный(ПараметрыЗапроса);
			ИнфоКурсовТоваров = ИнтерфейсHTTPЗапросовКлиентСервер.ПрочитатьОтветHTTPЗапроса(ТекстОтвета);
...
Функция ПолучитьКурсыВалютYahoo(ЗагружаемыеВалюты, ОсновнаяВалюта)
	...
	ОтветСтрока = ИнтерфейсHTTPЗапросовКлиентСервер.ПолучитьОтветHTTPЗапроса(ПараметрыЗапроса);
	
	НеформатированныеВалюты = ИнтерфейсHTTPЗапросовКлиентСервер.ПрочитатьОтветHTTPЗапроса(ОтветСтрока);
