Процедура ПерейтиНаВерсию_1_0_0_00() Экспорт 
	...
	КодРубляПоКлассификатору = "643";
	РегВалюта = Справочники.Валюты.НайтиПоКоду(КодРубляПоКлассификатору);
	Если РегВалюта = Справочники.Валюты.ПустаяСсылка() Тогда
		РегВалюта = Справочники.Валюты.СоздатьЭлемент();
		РегВалюта.Код = КодРубляПоКлассификатору;
		РегВалюта.Записать();
	КонецЕсли;
	
	Константы.ВалютаРегламентированногоУчета.Установить(РегВалюта);
	
	// Установка доп валюты.
	КодДоллара = "840";
	ВалютаДоллар = Справочники.Валюты.НайтиПоКоду(КодДоллара);
	Если ВалютаДоллар = Справочники.Валюты.ПустаяСсылка() Тогда
		ВалютаДоллар = Справочники.Валюты.СоздатьЭлемент();
		ВалютаДоллар.Код = КодДоллара;
